...
|---|---|---|
| `GET /state/card` | плагин | карточка (текст, без чисел) + маска полномочий `gate` |
...
`budget.initiation_enabled` в конфиге по умолчанию `false` (машинерия была,
доставки не было — включать только когда исполнитель развёрнут и получатель
настроен).
...
`forbidden: outbound` (например, RAGE), отправка всё равно будет отменена.

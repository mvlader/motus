...
| исполнитель задач (Tier 1) | `deploy/motus-tier1.{service,timer}` внутри `grach`, `User=openclaw`, код `/opt/motus-tier1/tier1_executor.py`; дёргает `GET /task/next` → `openclaw agent exec --config /opt/motus-tier1/exec-config.json` (модель `ge4b-heretic` на ПК, инструменты read/write/edit/exec/memory — без каналов отправки и без веба) → `POST /consummation`. Результаты — `~openclaw/.openclaw/workspace/.motus/drops/` |

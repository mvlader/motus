...
//     - если gate.forbidden содержит "outbound" (сейчас это только RAGE) — отменяет
...
//       ВНИМАНИЕ: отменяется ЛЮБАЯ отправка, включая ответ на прямой вопрос
//       человека, — это «режим молчит», а не «режим не пишет первым». Запрет
//       инициативы выражается через gate.may_initiate и только через него.
//       Вешать "outbound" на режим, который обязан отвечать, нельзя: на этом
//       уже погорел PLAY (исправлено 2026-09-15, см. gates.py и тест
//       TestForbiddenOutboundMeansSilence);

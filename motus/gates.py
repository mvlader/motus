...
REGIME_POLICY: Dict[str, Policy] = {
    ...
    # PLAY не даёт права перебивать: играть можно, вторгаться — нет. Ровно это
    # и означает may_initiate=False — и только это. "outbound" в forbidden
    # отсюда убран 2026-09-15: он значит не «не перебивай», а «не отправляй
    # ВООБЩЕ», то есть глушил и ответ на прямой вопрос человека. Двумя путями
    # сразу: фразой «Ничего не отправляй наружу» в карточке (она уходит модели
    # всегда, независимо от applyGate) и отменой отправки в хуке
    # message_sending плагина при applyGate=true. Запрет инициации живёт в
    # may_initiate, дублировать его через forbidden нельзя — это разные вещи.
    "PLAY":     Policy(False, 500, ("read", "memory", "write"), ()),

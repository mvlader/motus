...
список задач, которые бот может сам себе поставить в фоне, когда не разговаривает
с человеком (репертуар консумматорных актов). Ниже — текущие шаблоны и то, как
они реально сработали за последние сутки: mean_delta (насколько задача снижала
...
Тебе можно (и только это):
  ...
  * "add" — предложить новый шаблон: id (латиница/цифры/подчёркивание, строчными,
    ...
    consummation_type (один из: {", ".join(KNOWN_CONSUMMATION_TYPES)}),
